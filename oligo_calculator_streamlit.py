...
def main():
    ...
    # 从 session_state 中读取结果（这样上传 CSV 触发 rerun 时结果不会丢失）
    ...
    if results is not None:
        if not results["success"]:
            ...
            st.download_button(
                label="⬇️ 下载完整结果 CSV",
                data=csv,
                file_name=f"oligo_results_{results['sequence'][:20].replace('-','_')}.csv",
                mime="text/csv",
            )

            # ── 实测分子量匹配 ──
            st.divider()
            st.subheader("🔍 实测分子量匹配")
            st.caption(
                "上传包含实测分子量的 CSV 文件，自动与理论杂质分子量进行匹配。"
                "容差默认 ±0.5 Da，可自行调整。"
            )

            tol_col, upload_col = st.columns([1, 3])
            with tol_col:
                tolerance = st.number_input(
                    "容差 Tolerance (Da)",
                    min_value=0.01, max_value=10.0,
                    value=0.5, step=0.1, format="%.2f",
                    help="实测与理论 m/z 差值绝对值 ≤ 此值则判定为匹配",
                )
            with upload_col:
                uploaded_csv = st.file_uploader(
                    "上传实测分子量 CSV",
                    type=["csv"],
                    help="CSV 中应包含一列实测 m/z 数据（数值列）。"
                         "程序会自动识别数值列进行匹配。",
                )

            if uploaded_csv is not None:
                try:
                    obs_df = pd.read_csv(uploaded_csv)
                ...
                if obs_df is not None and not obs_df.empty:
                    # 找出所有数值列供用户选择
                    numeric_cols = obs_df.select_dtypes(include="number").columns.tolist()
                    if not numeric_cols:
                        st.error("CSV 中未找到数值列，请检查文件格式。")
                    else:
                        mw_col = st.selectbox(
                            "选择实测 m/z 所在的列",
                            numeric_cols,
                        ...
                        observed_values = obs_df[mw_col].dropna().tolist()

                        if not observed_values:
                            st.warning("所选列中没有有效数值。")
                        else:
                            # 汇总所有理论杂质（含各加成离子和流动相）
                            theory_list = []
                            z = results["charge"]
                            for section_label, section_data, pfx in [
                                (f"Standard (z={z})", results["std"],  f"{z}-"),
                                ("Mobile Phase",      results["mob"],  f"{z}-"),
                                ("Na-addition",       results["naA"],  "Na-"),
                                ("K-addition",        results["kA"],   "K-"),
                                ("Fe-addition",       results["feA"],  "Fe-"),
                            ]:
                                for name, mw in section_data:
                                    theory_list.append({
                            ...
                            match_rows = []
                            unmatched_obs = []
                            for obs_mw in observed_values:
                                matched = False
                                for t in theory_list:
                                    diff = obs_mw - t["理论 m/z"]
                                    if abs(diff) <= tolerance:
                                        match_rows.append({
                                            "实测 m/z": f"{obs_mw:.5f}",
                                            "理论杂质名称": t["理论杂质名称"],
                                            "理论 m/z": f"{t['理论 m/z']:.5f}",
                                            "差值 (Da)": f"{diff:+.5f}",
                                            "Section": t["Section"],
                                        })
                                        matched = True
                                if not matched:
                                    unmatched_obs.append(obs_mw)

                            # 显示结果
                            if match_rows:
                                st.success(
                                    f"共 {len(observed_values)} 个实测值，"
                                    f"匹配到 {len(match_rows)} 条记录 "
                                    f"（容差 ±{tolerance} Da）"
                                )
                                ...
                                # 导出匹配结果
                                match_csv = match_df.to_csv(
                                    index=False, encoding="utf-8-sig"
                                )
                                st.download_button(
                                    label="⬇️ 下载匹配结果 CSV",
                                    data=match_csv,
                                    file_name="mw_match_results.csv",
                                    mime="text/csv",
                                    key="dl_match",
                                )
                            else:
                                st.warning(
                                    f"在 ±{tolerance} Da 容差内未匹配到任何杂质。"
                                )

                            if unmatched_obs:
                                with st.expander(
                                    f"⚠️ 未匹配的实测值（{len(unmatched_obs)} 个）"
                                ):
                                    um_df = pd.DataFrame(
                                        {"未匹配 m/z": [f"{v:.5f}" for v in unmatched_obs]}
                                    )
                                    st.dataframe(
                                        um_df,
                                        hide_index=True,
